...
$.fn.wizard = function(config) {
    ...
    var exitText = config.exit || '종료';
    var backText = config.back || '뒤로';
    var nextText = config.next || '다음';
    var finishText = config.finish || '시작하기';
    ...
    if (isModal) {
        $(this).on('hidden.bs.modal', function () {
        	step = 1;
        	$($(containerSelector+" .wizard-steps-panel .step-number").removeClass("done").removeClass("doing")[0]).toggleClass("doing");
        	$($(containerSelector+" .wizard-step").hide()[0]).show();
